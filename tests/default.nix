...
# `testScript`) that flips some `itera.*` battery on (curated per-user program
# config lives at `itera.users.<name>.programs.<app>.*`) and asserts the
# resulting files/packages/env landed correctly.
#
  ...
  runOne = file: {
    ...
    value = nixos-lib.runTest {
      ...
      defaults = {
        ...
        # (tests/disko-impermanence-eval.nix) and interactive VM (dev/vm.nix);
        # turn them off for the framework's VM tests so every test node boots the
        # managed root.
        itera = {
          disko.enable = false;
        ...
        # A user for tests to configure — hjem home files land under
        # `hjem.users.test.*`, curated program config under
        # `itera.users.test.programs.<app>.*`.
        users.users.test = {
          isNormalUser = true;
          home = "/home/test";
        };
